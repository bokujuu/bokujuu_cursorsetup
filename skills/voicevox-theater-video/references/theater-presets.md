...
利用可能なパラメータを経験的に収束させた値を既定とする。案件で変えてよいが、変えるときは生成前に差分を一言残す。
...
| 素材候補 | `tachie-sources.md` の冥鳴ひまり優先版（全身 PSD） | `tachie-sources.md` の春日部つむぎ「さっぱり」優先版（全身 PSD） |

素材の安定した取得ページ、SHA-256、代替候補、取得・クレジット手順は [tachie-sources.md](tachie-sources.md) を正とする。案件側では PSD を正本として保持し、必要な表情・口状態だけを PNG にエクスポートする。
...
### 一人称は主語・目的語（文頭の飾りにしない）

`listener_first_person: あーし` は、つむぎが自分を指すときの一人称である。相槌・フィラー・相手への呼び掛けではない。`call_names` と「一致させる」ために毎発話の文頭へ置かない。

| 判定 | 例 | 理由 |
|------|----|------|
| 悪い | 「あーし、そっちを本命にするの？」 | 主語は相手。文頭の「あーし、」が相槌・呼び掛けに聞こえる |
| 良い | 「ひまっち、そっちを本命にするの？」 | 相手への呼び掛けは `listener_to_teacher` |
| 良い | 「あーしはこっちの方がしっくりくるんだけど」 | つむぎ自身が主語 |
| 良い | 「そっちを本命にするの？」 | 一人称が不要なら出さない |

teacher の「私」も同じ。相手の行為を問う文の文頭に「私、」を置かない。
...
登場／退場の詳細は各演出資料に分け、ここではプロファイル既定値だけを SoT とする。要約:
...
- [ ] 呼称の値を使うなら `call_names` と一致。一人称は主語・目的語のときだけ（文頭の「あーし、」が相槌になっていない）
