...
description: >-
  Rewrites Japanese prose for naturalness and readability (AI-smell, translationese,
  unclear sentences). Use for 仕事文書, notes, essays, 「自然にして」, AI臭さ.
  Not for technical-doc structure (japanese-technical-writing), review-only
  findings (japanese-doc-review), or essay cognitive rhythm (cognitive-rhythm-writing).
license: MIT
...
本スキルは文章の自然さ・AI臭さ・読みやすさに特化する。章構成テンプレート、指摘レビュー、認知リズム、動画台詞は隣へ委譲する。

| 依頼 | 先に使う skill | 本スキルの位置 |
|------|----------------|----------------|
| 技術説明・設計書・手順の型と常体 | `../japanese-technical-writing/SKILL.md` | 型が決まったあと `--genre tech` で自然さ・AI臭を見る |
| 「レビューして」「問題点を指摘して」（指摘のみ） | `../japanese-doc-review/SKILL.md` | 使わない。「自然にして」は本スキル |
| 読み物の緩急・未回収の緊張・駄文 | `../cognitive-rhythm-writing/SKILL.md` | 両方なら本スキルで自然さを収束してから緩急を載せる |
| スライド動画・台詞 | `../slide-narration-video/SKILL.md` | 本スキルの slide doctype は構成案の文章だけ |
| チャット応答の口調 | User Rules（です／ます） | 文書本文には当てない |

衝突時: 技術文書の常体・太字禁止は `japanese-technical-writing` を優先。dialogue 台詞の表面文体は `slide-narration-video` の `dialogue-writing` を優先。

## 実行環境

検査スクリプトは `uv run`（PEP 723、sudachipy）を前提とする。`uv` が無い、または第三者パッケージを入れられない環境では `references/manual-checklist.md` で同じ観点を人手でなぞる。`semantic.py` は torch 依存のため、ユーザーが `exp` / 深層検出を指定しない限り走らせない。コマンドは本 skill ディレクトリをカレントにして実行する。
