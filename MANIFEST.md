...
| `fable` | Fable 型の観測優先エージェント推論（三原則・Phase 0–4・証拠付き完了）。着想: [shotatykr の /fable 構想](https://x.com/shotatykr/status/2074035238116769851)。公式開示は [Anthropic System Prompts — Fable 5](https://platform.claude.com/docs/en/release-notes/system-prompts) を参照（手順本体はコミュニティトレース）。併用: `agent-handoff-recovery` / `ralph-loop` / `retrospective-codify` |
