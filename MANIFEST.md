...
| `requirement-aligned-fixtures` | 要件・スキーマに沿ったダミーデータ設計。三軸バランス（決定性・変動・グループ）、volume tier、カバレッジ行列、Generation Spec・manifest。表形式・PCAF 型 Excel 素データは `references/tabular-excel.md`。帳票品質は `excel-deliverable-quality` と併用 |
