...
| **COM/Excel（win32com）** | skill `excel-deliverable-quality`（スクリプト生成・役割配色・印刷規律・検証） → 対象リポの `.cursor/rules/excel-com-automation.mdc`（API 手順・雛形: `templates/project-rules/excel/`）。層 B: python-coding-policy |
| **Excel/CSV 成果物の品質・レイアウト・配色** | skill `excel-deliverable-quality`（`.xlsm` 手編集しない／入力=黄・計算=灰の役割配色・印刷範囲・1枚化／数式エラーゼロ・数式優先・納品前検証） |
...
| `excel-deliverable-quality` | Excel/CSV 成果物の品質・レイアウト・配色規約・納品前検証（スクリプト生成・役割配色・印刷規律。COM 第一・openpyxl+LibreOffice 代替） |
